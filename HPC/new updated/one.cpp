...
int main() {
    ...
    std::cout << "Time taken for Sequential BFS: " << (end - start) << " seconds\n\n";

    // Test Parallel BFS
    start = omp_get_wtime();
    g.parallelBFS(0);
    end = omp_get_wtime();
    std::cout << "Time taken for Parallel BFS: " << (end - start) << " seconds\n\n";

    // Test Sequential DFS
    start = omp_get_wtime();
    g.sequentialDFS(0);
    end = omp_get_wtime();
    std::cout << "Time taken for Sequential DFS: " << (end - start) << " seconds\n\n";

    // Test Parallel DFS
    start = omp_get_wtime();
    g.parallelDFS(0);
    end = omp_get_wtime();
    std::cout << "Time taken for Parallel DFS: " << (end - start) << " seconds\n\n";
